  ...
  try {
    switch (providerName) {
      case 'moonshot':
        ...
        // 支持两种响应格式：{ data: {...} } 或直接 { code: 0, data: {...} }
        if (result.success) {
          const respData = result.data.data || result.data;
          if (respData && (respData.available_balance !== undefined || respData.cash_balance !== undefined)) {
            return {
              success: true,
              provider: config.name,
              balance: respData.available_balance || respData.cash_balance || 0,
              cash: respData.cash_balance || 0,
              voucher: respData.voucher_balance || 0,
              currency: 'CNY'
            };
          }
        }
        return { success: false, error: result.error || 'Invalid response', raw: result.raw };
...
async function main() {
  ...
  switch (command) {
    ...
    case 'balance':
      ...
      // 支持多种环境变量格式：MOONSHOT_API_KEY, moonshot_api_key
      const key = args[2] || process.env.MOONSHOT_API_KEY || process.env.moonshot_api_key || process.env.OPENAI_API_KEY;
